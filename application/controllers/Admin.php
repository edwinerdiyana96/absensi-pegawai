...
class Admin extends CI_controller
{
    ...
    public function importExcelUsers()
    {
        ...
        try {
            ...
            foreach ($sheet as $index => $row) {
                ...
                $data[] = [
                    'name' => $row['A'],
                    'email' => $row['B'],
                    'password' => password_hash($row['C'], PASSWORD_BCRYPT), // Hash password
                    'gender' => $row['D'],
                    'role_id' => $row['E'],
                    'department' => $row['F'],
                    'phone' => $row['G'],
            ...
            // Simpan ke database
            $this->Admin_model->insertBatchUsers($data);
        ...
        $this->session->set_flashdata('message', '<script>
         swal({
            ...
            text: "Import Data Dari Excel Berhasil!",
            icon: "success",
            button: "Ok"
                // timer: 3000
            });
    ...
    public function delete_all_data_users() {
    $this->Admin_model->delete_all_users_except_admin();

        // Set flash message dan redirect
        $this->session->set_flashdata('message', '<script>
         swal({
            title: "Hapus Semua Data User",
            text: "Semua data pengguna berhasil dihapus!",
            icon: "success",
            button: "Ok"
                // timer: 3000
            });
...
//update user 
    ...
    if ($params == 'logo') {
        ...
        if (!$this->upload->do_upload('upload')) {
            ...
            swal({
               title: "Berhasil!",
               text: "Foto Sampul Berhasil diperbarui",
               icon: "success",
               button: "Ok"
            });
            </script>');
        ...
        swal({
           title: "Berhasil!",
           text: "Data Pengaturan Berhasil diperbarui",
           icon: "success",
           button: "Ok"
        });
        </script>');
    ...
    public function settings_api($params="", $params2=""){
        if ($params=='edit') {
            ...
            $input = [
                ...
                'wa_target'  => $this->input->post('wa_target'),
                'bot_telegram'  => $this->input->post('bot_telegram'),
                'token_telegram'  => $this->input->post('token_telegram'),
