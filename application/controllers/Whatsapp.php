...
class Whatsapp extends CI_controller
{
    ...
    public function group()
	{
		...
		$this->load->view('whatsapp/group', $data);
	...
	public function sendRekapWhatsapp($params = "")
    {
		...
		$title1 = "Guru / Pegawai Yang \n*BELUM HADIR / SCAN:*".$enter;
...
', $str2_off);
        ...
        if ($params == 'group') {
            ...
            curl_exec($curl);
            curl_close($curl);
            redirect('operator/kehadiran');
        } elseif ($params == 'pribadi') {
            ...
            curl_setopt_array(
                ...
                array(
                    CURLOPT_URL => 'https://api-whatsapp.smkkarnas.id/sendtext?number=' . $phone . '&to=' . $group . '&message=' . $text,
                    // CURLOPT_URL => 'https://api-whatsapp.smkkarnas.id/sendtext?number=' . $phone . '&to=' . $whatsapp . '&message=' . $text,
                    CURLOPT_RETURNTRANSFER => true,
                    CURLOPT_ENCODING => '',
                    CURLOPT_MAXREDIRS => 10,
                    CURLOPT_TIMEOUT => 0,
            ...
            $whatsapp = $settings['wa_target'] . "@s.whatsapp.net";
            $curl = curl_init();
            curl_setopt_array(
                $curl,
                array(
...
', $str2);
    ...
    public function send_tepat_waktu()
    {
        $settings = $this->db->get('settings')->row_array();
        $data['tepat_waktu'] = $this->Absensi_model->getNotLateAttendanceToday()->result_array();

        $enter = " " . chr(10);
        $garis = "------------------------------------------------------";
        $info =  "[PESAN OTOMATIS] - ";
        $date = tanggal_indo(date('Y-m-d'), true) . $enter;
        $title = "Guru / Pegawai Yang Tepat Waktu (Realtime):".$enter;
        ...
        foreach ($data['tepat_waktu'] as $key => $tptwkt) {
            $phpdate = strtotime($tptwkt['time_in']);
            $mysqldate = date('H.i.s', $phpdate);
            $no = $key + 1;
            $arr[] = array(
                '' => $no,
...
', $str2);
        ...
        $text = urlencode($info . $date . $garis . $enter . $title . $str2);
        $whatsapp = '6282215293636@s.whatsapp.net';
        $phone = $settings['phone'];
        $curl = curl_init();
        curl_setopt_array(
            $curl,
