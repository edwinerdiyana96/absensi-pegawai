...
class Absensi_model extends CI_Model
{
	...
	public function getHadirKhususBulanById($params = "", $awal = "", $akhir = "")
	{
		$this->db->where('date <=', $akhir);
		$this->db->where('date >=', $awal);
		$this->db->where('user_id', $params);
	...
	public function getNotLateAttendanceToday()
	{
		// date_default_timezone_set("Asia/Jakarta");
		// $date = new DateTime("now");
		// $curr_date = $date->format('Y-m-d');
	...
	public function absen()
	{
		...
		if ($curr_time >= $jam_masuk_start && $curr_time <= $jam_masuk_end) { //jam masuk tepat waktu
			...
			if ($time_in['time_in'] == $time_cek) { // cek jika belum 
                ...
                $curl,
                array(
                    CURLOPT_URL => 'https://absensi-pegawai-kuningan.smkkarnas.id/whatsapp/send_tepat_waktu/',
		...
		if ($time_user_cek == 0) {
			...
			if ($curr_time < $time_cek2) {
				$this->session->set_flashdata('message', '<script>
                   swal({
                    title: "Status Absen!",
                    text: "Anda Baru Saja Melakukan Absen Masuk, Harap Tunggu 10 Menit Untuk Melakukan Absen Berikutnya!",
                    icon: "warning",
                    button: "OK",
					timer: 2000
					});
